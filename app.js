(() => {
  ...
  function loadState() {
    try {
      ...
      return {
        ...
        wordStats: normalizeWordStats(parsed.wordStats || {}),
    ...
    main.innerHTML = `
      ...
      <section class="section">
        ...
        <div class="button-stack">
          ...
          <button class="secondary-button full" id="showMistakes">復習リストを見る</button>
    ...
    main.innerHTML = `
      <section class="card">
        ...
        <div class="button-stack" style="margin-top:20px">
          ...
          <button class="secondary-button full" id="startReview">復習リストから練習</button>
        </div>
      </section>
    `;
  ...
  function startSession(mode) {
    ...
    if (mode === "review") {
      ...
      if (!source.length) {
        const fallback = getReviewWords().find((item) => item.stat.needsReview);
        if (fallback) {
  ...
  function handleAnswer(selected, correct, word, question) {
    ...
    updateWordStat(word.id, isCorrect, activeSession.mode, activeSession.language);
    saveState();
    saveActiveSession();
    ...
    const alreadyInReviewList = isWordInReviewList(word.id);
    ...
    feedback.innerHTML = `
      <div class="feedback ${isCorrect ? "success" : "error"}">
        ...
        <button class="secondary-button full" id="addReviewButton" type="button" ${alreadyInReviewList ? "disabled" : ""}>
          ${alreadyInReviewList ? "復習リストに追加済み" : "復習リストに追加"}
        </button>
        ${studyLinks}
    ...
    if (addReviewButton && !alreadyInReviewList) {
      addReviewButton.addEventListener("click", () => {
        addWordToReviewList(word.id, activeSession.language);
        response.addedToReviewList = true;
        saveActiveSession();
        addReviewButton.textContent = "復習リストに追加済み";
  ...
  function createWordStat() {
    return {
      ...
      lastAnsweredAt: null,
      lastWrongAt: null,
      lastReviewAddedAt: null
    };
  }

  function normalizeWordStats(stats) {
    const normalized = {};
    for (const [wordId, rawStat] of Object.entries(stats || {})) {
      const stat = { ...createWordStat(), ...(rawStat || {}) };
      if (typeof rawStat?.inReviewList !== "boolean") {
        stat.inReviewList = Boolean(rawStat?.everWrong || rawStat?.needsReview);
      }
      stat.reviewLanguages = Array.isArray(rawStat?.reviewLanguages)
        ? [...new Set(rawStat.reviewLanguages)]
        : [];
      normalized[wordId] = stat;
    }
    return normalized;
  }

  function getOrCreateWordStat(wordId) {
    const existing = state.wordStats[wordId];
    if (existing) {
      state.wordStats[wordId] = { ...createWordStat(), ...existing };
      if (!Array.isArray(state.wordStats[wordId].reviewLanguages)) {
        state.wordStats[wordId].reviewLanguages = [];
      }
      return state.wordStats[wordId];
    }
    state.wordStats[wordId] = createWordStat();
    return state.wordStats[wordId];
  }

  function addReviewLanguage(stat, language) {
    if (!language) return;
    if (!Array.isArray(stat.reviewLanguages)) stat.reviewLanguages = [];
    if (!stat.reviewLanguages.includes(language)) stat.reviewLanguages.push(language);
  }

  function isWordInReviewList(wordId) {
    return Boolean(state.wordStats[wordId]?.inReviewList);
  }

  function addWordToReviewList(wordId, language) {
    const stat = getOrCreateWordStat(wordId);
    stat.inReviewList = true;
    stat.needsReview = true;
    stat.reviewStreak = 0;
    stat.lastReviewAddedAt = new Date().toISOString();
    addReviewLanguage(stat, language);
    saveState();
  }

  function updateWordStat(wordId, isCorrect, mode, language) {
    const existing = getOrCreateWordStat(wordId);

    if (existing.inReviewList) addReviewLanguage(existing, language);
    ...
    if (isCorrect) {
      ...
      existing.inReviewList = true;
      existing.needsReview = true;
      existing.reviewStreak = 0;
      existing.lastWrongAt = new Date().toISOString();
      existing.lastReviewAddedAt = existing.lastWrongAt;
      addReviewLanguage(existing, language);
    }
  ...
  function renderResult() {
    ...
    main.innerHTML = `
      ...
      <section class="section">
        <div class="section-title"><h2>今回、復習する単語</h2><p>${wrong.length}語</p></div>
      ...
      <section class="section button-stack">
        ...
        <button class="secondary-button full" id="resultReview" ${getNeedsReviewCount() ? "" : "disabled"}>復習リストから練習</button>
  ...
  function renderMistakes() {
    const allReviewWords = getReviewWords();
    normalizeReviewFilter(allReviewWords);

    const filtered = mistakeFilter === "all"
      ? allReviewWords
      : allReviewWords.filter((item) => item.language === mistakeFilter);
    const isSentenceMode = reviewDisplayMode === "sentences";
    const title = isSentenceMode ? "例文を暗記" : "復習する単語";
    const listHtml = isSentenceMode
      ? filtered.map((item) => reviewSentenceItemHtml(item)).join("")
      : filtered.map((item) => reviewWordItemHtml(item)).join("");

    main.innerHTML = `
      <section>
        <div class="section-title"><h2>${title}</h2><p>${filtered.length}件</p></div>
        <div class="filter-row" id="reviewDisplayModes">
          ${reviewModeButton("words", "単語リスト")}
          ${reviewModeButton("sentences", "例文暗記")}
        </div>
      </section>
      ...
      <section class="section">
        <div class="notice">
          ${isSentenceMode
            ? "日本語の例文をタップすると、外国語の例文を表示します。"
            : "単語をタップすると、和訳、例文、追加学習メニューを表示します。"}
        </div>
      </section>

      <section class="section">
        <div class="filter-row" id="reviewLanguageFilters">
          ${reviewLanguageFilterButtons(allReviewWords, isSentenceMode)}
        </div>
      </section>
      ...
      <section class="section">
        ${filtered.length
          ...
          : `<div class="card empty"><span class="emoji">✓</span>この条件の復習単語はありません。</div>`}
      </section>
    ...
    document.querySelectorAll("[data-review-display]").forEach((button) => {
      button.addEventListener("click", () => {
        reviewDisplayMode = button.dataset.reviewDisplay;
        renderMistakes();
      });
    });
    ...
    document.querySelectorAll("[data-review-filter]").forEach((button) => {
      button.addEventListener("click", () => {
        mistakeFilter = button.dataset.reviewFilter;
        renderMistakes();
      });
    });
  ...
  function reviewModeButton(value, label) {
    return `<button class="filter-button ${reviewDisplayMode === value ? "active" : ""}" data-review-display="${escapeAttr(value)}">${escapeHtml(label)}</button>`;
  }

  function reviewLanguageFilterButtons(items, sentenceMode) {
    const buttons = [];
    if (!sentenceMode) buttons.push(reviewFilterButton("all", "すべて"));

    const languageOrder = ["zh-Hans", "zh-Hant", "ko", "eo", "fi"];
    const shortLabels = {
      "zh-Hans": "中国語・簡体",
      "zh-Hant": "中国語・繁体",
      "ko": "韓国語",
    ...
    const available = new Set(items.map((item) => item.language));
    languageOrder.forEach((language) => {
      if (available.has(language)) buttons.push(reviewFilterButton(language, shortLabels[language]));
    });
    return buttons.join("");
  }

  function reviewFilterButton(value, label) {
    return `<button class="filter-button ${mistakeFilter === value ? "active" : ""}" data-review-filter="${escapeAttr(value)}">${escapeHtml(label)}</button>`;
  }

  function normalizeReviewFilter(items) {
    const available = [...new Set(items.map((item) => item.language))];
    if (reviewDisplayMode === "sentences") {
      if (mistakeFilter === "all" || !available.includes(mistakeFilter)) {
        mistakeFilter = available.includes(state.settings.language)
          ? state.settings.language
          : (available[0] || state.settings.language);
      }
    } else if (mistakeFilter !== "all" && !available.includes(mistakeFilter)) {
      mistakeFilter = "all";
    }
  }

  function reviewWordItemHtml(item) {
    ...
    return `<details class="history-item">
      <summary>
        <div class="history-summary">
          <div>
            <strong>${escapeHtml(foreignWord)}</strong>
            ${pinyin ? `<small style="display:block;margin-top:4px">${escapeHtml(pinyin)}</small>` : ""}
            <small style="display:block;margin-top:4px">${escapeHtml(languageLabels[item.language])}・間違い ${stat.incorrect}回・正解 ${stat.correct}回・タップして追加学習</small>
          </div>
          <span class="badge ${stat.needsReview ? "" : "mastered"}">${stat.needsReview ? "要復習" : "復習済み"}</span>
        </div>
      </summary>
      <div class="history-detail">
        <p class="word-line" style="margin-top:0">${escapeHtml(foreignWord)} ＝ ${escapeHtml(item.word.japanese)}</p>
        <div class="example-box">
          <p>${escapeHtml(example.foreign)}</p>
          <p>${escapeHtml(example.japanese)}</p>
        </div>
  ...
  function reviewSentenceItemHtml(item) {
    const example = getExample(item.word, item.language);
    const studyLinks = buildStudyLinksHtml(item.word, item.language);
    ...
    return `<details class="history-item">
      <summary>
        <div>
          <strong>${escapeHtml(example.japanese)}</strong>
          <small style="display:block;margin-top:6px;color:var(--muted)">タップして外国語の例文を表示</small>
        </div>
      </summary>
      <div class="history-detail">
        <div class="example-box" style="margin-top:0">
          <p style="font-size:1.05rem;font-weight:800;color:var(--text)">${escapeHtml(example.foreign)}</p>
          <p>${escapeHtml(example.japanese)}</p>
        </div>
        ${studyLinks}
      </div>
  ...
  function getReviewWords() {
    ...
    for (const [wordId, stat] of Object.entries(state.wordStats)) {
      if (!stat.inReviewList && !stat.everWrong) continue;
      let language;
      if (wordId.startsWith("zh")) {
        const recordedLanguages = Array.isArray(stat.reviewLanguages)
          ? stat.reviewLanguages.filter((lang) => lang.startsWith("zh"))
          : [];
        const appearances = recordedLanguages.length ? recordedLanguages : getChineseAppearances(wordId);
        [...new Set(appearances)].forEach((lang) => {
    ...
    return result.sort((a, b) => {
      const aDate = a.stat.lastReviewAddedAt || a.stat.lastWrongAt || a.stat.lastAnsweredAt || "";
      const bDate = b.stat.lastReviewAddedAt || b.stat.lastWrongAt || b.stat.lastAnsweredAt || "";
  ...
  function getChineseAppearances(wordId) {
    ...
    if (activeSession?.responses) sessions.unshift(activeSession);
    for (const session of sessions) {
      if (session.language?.startsWith("zh") && session.responses?.some((r) => r.wordId === wordId)) {
  ...
  function renderSettings() {
    main.innerHTML = `
      <section class="card">
        ...
        <div class="setting-row">
          <h3>すべての学習記録を削除</h3>
          <p>この端末内の成績、復習リスト、履歴を削除します。元に戻せません。</p>
    ...
    reader.onload = () => {
      try {
        ...
        state = {
          ...
          wordStats: normalizeWordStats(incoming.wordStats),
